...
kotlin {
    ...
    sourceSets {
        ...
        androidMain.dependencies {
            ...
            // Бэкенд EdDSA (ADR-016) — implementation, а не compileOnly:
            // ed25519 нужен в рантайме и нам (генерация ключа), и самому sshd
            // (кодирование ключа, подпись рукопожатия), а records, из-за
            // которых JGit пришлось делать compileOnly, здесь нет — дексация
            // device-тестов проходит. Приложению видеть его незачем: работа
            // идёт через SshKeyStore.
