 ...
 *
 * *Путь одиночного файла уступает место tree-доступу* (`SL-7`, решение
 * владельца от 2026-08-17: сценарий открытия — «выбрать папку, затем файл в
 * ней»). Замена — [AndroidDocumentTreeAccess]: тот же контракт
 * [DocumentFileAccess] для открытия и записи, но право удерживается на дерево,
 * и рядом появляются перечень документов папки и чтение соседних картинок.
 * Этот класс сохраняется с прежними сигнатурами, пока экран строится на нём;
 * после интеграции экрана с tree-путём он уходит. Взятие права здесь и в
 * tree-доступе взаимно вытесняющее: правило «ровно одно право» действует на
 * приложение, а не на каждый путь по отдельности.
 */
class AndroidDocumentFileAccess(context: Context) : DocumentFileAccess {
    ...
    fun takePersistableAccess(uri: Uri): DocumentSource {
        ...
        // Прежние разрешения отдаются, иначе они копятся: система держит не
        ...
        // после четырёх открытий в списке лежали все четыре. Отдаются *все*
        // прочие, а не только запомненное здесь: с появлением tree-доступа
        // (`SL-7`) правило «ровно одно право» действует на приложение, и право
        // на дерево, взятое другим путём, тоже считается.
