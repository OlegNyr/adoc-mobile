// --- Добавлено patch-asciidoctor.py: окружение, которого нет в QuickJS ---
...
//   - ROOT_DIR / LIB_DIR / DATA_DIR / USER_HOME – computed via '' below
...
let USER_HOME = '';
try {
  LIB_DIR = new URL('.', '').pathname.replace(/\/$/, '');
  ROOT_DIR = new URL('../../', '').pathname.replace(/\/$/, '');
  DATA_DIR = new URL('../../data', '').pathname;
...
globalThis.Asciidoctor = {
  ...
  ContentModel,
  ConverterBase,
  ConverterCustomFactory: CustomFactory$1,
  ConverterFactory: Converter,
  Cursor,
  DefaultConverterFactory: DefaultFactory$1,
  DefaultSyntaxHighlighterFactory: DefaultFactory,
  DocinfoProcessor,
  Document,
  DocumentTitle,
  Extensions,
