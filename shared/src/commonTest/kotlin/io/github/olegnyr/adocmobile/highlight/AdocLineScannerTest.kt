...
class AdocLineScannerTest {
    ...
    fun TC_10_sevenSignsAreNotAHeading() {
        ...
        // До `SL-3` оракул был «диапазонов нет вовсе», но у решётки есть второй
        // смысл — пара выделения. Прогон эталона по этой самой строке:
        // `####### Семь решёток` → `<mark><mark></mark></mark># Семь решёток` —
        // абзац получает две вложенные пары `##…##` и `#…#`. Заголовком не
        // становится ни одна из строк — в этом и состоит смысл кейса.
        assertEquals(
            listOf(
                AdocSpan(src.inLine(2, 0, 5), AdocStyle.Highlight),
                AdocSpan(src.inLine(2, 2, 6), AdocStyle.Highlight),
            ),
            src.scan().spans,
        )
    }
