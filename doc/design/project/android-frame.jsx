...
function AndroidKeyboard({ dark = false }) {
  let _k = 0;
  const K = dark
    ? { ground: '#101317', key: '#1c2126', mod: '#171b1f', enter: '#5980a6', enterFg: '#0d1318', fg: '#c6cbd0' }
    : { ground: MD_C.inverseOnSurface, key: MD_C.surface, mod: MD_C.surfaceVariant, enter: MD_C.primaryFixedDim, enterFg: MD_C.onPrimaryContainer, fg: MD_C.onPrimaryContainer };
  const key = (l, { flex = 1, bg = K.key, r = 6, minW, fs = 21, fg = K.fg } = {}) => (
    <div key={_k++} style={{
  ...
  return (
    <div style={{
      background: K.ground, padding: '0 8px 8px',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {key('?123', { bg: K.mod, r: 100, minW: 58, fs: 14 })}
          {key(',', { bg: K.mod })}
          {key('', { flex: 3, minW: 154 })}
          {key('.', { bg: K.mod })}
          {key('', { bg: K.enter, r: 100, minW: 58, fg: K.enterFg })}
        </div>
      </div>
    </div>
  );
