...
- **Apache MINA sshd looks up its EdDSA backend by reflection and, finding none, silently reports ed25519 as unsupported.** `SecurityUtils.isEDDSACurveSupported()` returns false and `PublicKeyEntry.toString` throws `Cannot retrieve decoder for key=EdDSA` — so the key generates and displays but can never be presented to a server. The fix is a backend on the classpath (`net.i2p.crypto:eddsa`, +32 KB in the APK; BouncyCastle is 135× larger for the same job — ADR-016), plus naming the provider explicitly at `KeyPairGenerator`/`KeyFactory` rather than registering it globally. Same failure mode as the `META-INF` packaging traps: green build, dead feature, discovered only by running it (2026-08-19).
